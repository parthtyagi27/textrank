...
class TextRank:
	...
	def __preprocess_data(self):
		...
		self.probabilities = np.zeros(self.dimension)
		counter = 1.0
		for token in self.tokens:
			self.probabilities[self.token_list.index(token)] += (float) (1.0 / counter)
			counter = counter + 1
			print(str(token) + " = " + str(self.probabilities[self.token_list.index(token)]))
	...
	def analyze(self, max_limit=10):
		...
		self.score_vector = np.ones(self.dimension) / self.dimension

		for step in range(self.iterations):
			self.score_vector = (1 - self.d) * self.probabilities + self.d * np.dot(graph, self.score_vector)
